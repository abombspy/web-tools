    ...
    blockingErrors.length === 0
      ? candidates.map((c) => {
          ...
          if (Number.isNaN(resignation.getTime())) {
            return { candidate: c, error: "날짜 형식이 올바르지 않습니다.", result: null };
          }
          if (hire && resignation.getTime() < hire.getTime()) {
            return { candidate: c, error: "퇴사일이 입사일보다 빠릅니다.", result: null };
          }
          return {
            candidate: c,
            error: null,
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <label className="flex flex-col gap-1 text-sm">
          ...
          <span className="text-xs text-zinc-500">
            최근 3개월간 동일했다고 가정합니다. 상여금 등 비정기 수입은 포함하지 않습니다.
          </span>
        </label>
      </div>
      ...
      <div className="mt-6 border-t-2 border-orange-100 pt-6 dark:border-white/10">
        ...
        <div className="space-y-3">
          {rows.length === 0 && blockingErrors.length === 0 && (
            <p className="text-sm text-zinc-500">위에 입사일과 월급을 먼저 입력해 주세요.</p>
          )}
              ...
              >
                ...
                {row?.result && (
                  <dl className="mt-3 grid grid-cols-2 gap-x-4 gap-y-1 text-sm sm:grid-cols-4">
                    <div>
                      <dt className="text-zinc-500">근속일수</dt>
                      <dd className="font-medium">{row.result.continuousServiceDays.toLocaleString()}일</dd>
                    </div>
                    <div>
                      <dt className="text-zinc-500">퇴직금 대상</dt>
                      <dd className="font-medium">
                        {row.result.severancePayEligible ? "O (1년 이상)" : "X (1년 미만)"}
                      </dd>
                    </div>
                    <div>
                      <dt className="text-zinc-500">예상 퇴직금</dt>
                      <dd className="font-medium">{row.result.severancePay.toLocaleString()}원</dd>
                    </div>
                    <div>
                      <dt className="text-zinc-500">발생 연차</dt>
                      <dd className="font-medium">{row.result.annualLeaveDays}일</dd>
                    </div>
                  </dl>
                )}
              </div>
