...
export default function Calculator() {
  ...
  if (courses.some((c) => !Number.isFinite(c.credit) || c.credit < 0)) {
    blockingErrors.push("Please enter credits as a number of 0 or more.");
  }
    ...
    <div className="mt-8 rounded-2xl border-2 border-indigo-100 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-zinc-900">
      <div className="space-y-3">
        {rows.map((row, idx) => (
          <div key={row.id} className="flex items-center gap-3">
            <span className="w-16 shrink-0 text-sm text-zinc-500">Course {idx + 1}</span>
            <input
              type="number"
              inputMode="numeric"
      ...
      {result && (
        <div className="mt-6 grid grid-cols-3 gap-4 border-t-2 border-indigo-100 pt-6 dark:border-white/10">
          <div>
            <p className="text-sm text-zinc-500">4.5 scale</p>
            <p className="text-2xl font-bold">{result.gpa45}</p>
          </div>
          <div>
            <p className="text-sm text-zinc-500">4.3 scale (converted)</p>
