...
export default function Calculator({ rates }: { rates: RealEstateAgentFeeRates }) {
  ...
  if (type === "lease" && (!Number.isFinite(depositNum) || depositNum < 0)) {
    blockingErrors.push("Please enter a deposit of 0 or more.");
  }
  if (type === "lease" && (!Number.isFinite(monthlyRentNum) || monthlyRentNum < 0)) {
    blockingErrors.push("Please enter monthly rent of 0 or more.");
  }
    ...
    <div className="mt-8 rounded-2xl border-2 border-sky-100 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-zinc-900">
      <label className="mb-4 flex flex-col gap-1 text-sm">
        ...
        >
          ...
          <option value="lease">Jeonse / monthly rent (lease)</option>
        </select>
      </label>
      ...
      {result && (
        <div className="mt-6 space-y-3 border-t-2 border-sky-100 pt-6 dark:border-white/10">
          <p className="text-sm text-zinc-500">Maximum brokerage fee (VAT separate)</p>
          <p className="text-3xl font-bold">₩{result.fee.toLocaleString()}</p>
          <ul className="mt-2 space-y-0.5 text-sm text-zinc-600 dark:text-zinc-400">
            {type === "lease" && monthlyRentNum > 0 && (
              <li>Converted deposit: ₩{result.transactionAmount.toLocaleString()}</li>
            )}
            <li>Applied rate: {result.appliedRatePercent}%</li>
            {result.cap !== null && <li>Cap: ₩{result.cap.toLocaleString()}</li>}
            {result.feeCapped && <li className="text-amber-600 dark:text-amber-400">The cap was applied.</li>}
          </ul>
        </div>
      )}
    </div>
