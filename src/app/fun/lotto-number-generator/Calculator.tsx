  ...
  return (
    <div className="mt-8 rounded-2xl border-2 border-violet-100 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-zinc-900">
      <label className="flex flex-col gap-1 text-sm">
        게임 수
        <select
          className="w-32 rounded-xl border-2 border-zinc-200 px-3 py-2 focus:border-violet-400 focus:outline-none dark:border-white/20 dark:bg-transparent"
          value={gameCount}
      ...
      <div className="mt-6 border-t-2 border-violet-100 pt-6 dark:border-white/10">
        <p className="text-sm font-medium">
          번호별 출현 빈도 (1~{frequency.asOfDrawNo}회, {frequency.asOfDrawDate} 기준)
        </p>
        <p className="mt-1 text-xs text-zinc-500">
          참고용일 뿐 다음 회차 예측과 무관합니다. 위 설명을 참고해 주세요.
        </p>
        <div className="mt-3 grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <p className="text-xs font-semibold text-zinc-500">가장 많이 나온 번호 TOP 5</p>
            <ul className="mt-2 space-y-1">
              {mostFrequent.map(([n, count]) => (
                <li key={n} className="flex items-center gap-2 text-sm">
                  <Ball n={Number(n)} />
                  <span className="text-zinc-600 dark:text-zinc-400">{count}회</span>
                </li>
              ))}
            </ul>
          </div>
          <div>
            <p className="text-xs font-semibold text-zinc-500">가장 적게 나온 번호 TOP 5</p>
            <ul className="mt-2 space-y-1">
