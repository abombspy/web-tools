...
// 관계마다 고정 점수 밴드 + 설명(소시오닉스 이론 기반, 원문 그대로 베끼지 않고
// 재작성) — 완전히 결정론적이라 "왜 이 점수인지" 설명 가능.
const RELATION_SCORE: Record<SocionicsRelation, number> = {
  duality: 95, activity: 85, mirror: 80, identity: 75, semiDuality: 70,
  kindred: 65, business: 60, mirage: 55, quasiIdentity: 45, contrary: 40,
  benefactor: 55, beneficiary: 50, supervisor: 30, supervisee: 25,
  superEgo: 20, conflict: 10,
};

export function calculateMbtiCompatibility(a: MbtiType, b: MbtiType) {
  const relation = determineRelation(a, b);
  return { relation, score: RELATION_SCORE[relation], stackA: FUNCTION_STACK[a], stackB: FUNCTION_STACK[b] };
}
```
...
- **점수 밴드는 예시일 뿐** — 16개 관계를 어떤 순서로 점수화할지(위 표는 "조화로울수록
  높은 점수"라는 상식적 순서를 임의로 매긴 것)는 §3에서 사용자 확인이 필요하다.
